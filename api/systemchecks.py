"""
The :py:mod:`api` application ships with some custom system
checks which ensure that required settings are present. These system checks are registered by
the :py:class:`~api.apps.Config` class's
:py:meth:`~api.apps.AssetsConfig.ready` method.
